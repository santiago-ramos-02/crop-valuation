...
test("pre-equilibrium appraisal keeps a positive net recovery value when current utility is negative", async () => {
  ...
  assert.equal(appraisal.pendingRecoveryCopHa, 1100000)
  assert.equal(appraisal.decisionTreeValueCopHa, 100000)
  assert.equal(appraisal.appraisalRule, "pre_equilibrium")
  assert.equal(appraisal.startedProducing, true)
  assert.equal(appraisal.appraisedValueCopHa, 100000)
  assert.equal(appraisal.appraisedValueCop, 100000)
})
